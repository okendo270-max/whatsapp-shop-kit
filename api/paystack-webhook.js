...
export default async function handler(req, res) {
  ...
  try {
    ...
    // 2) Match orders.order.order.order_id === reference (in case you stored reference there)
    if (!order && reference) {
      const { data: o2, error: o2err } = await supabaseAdmin
        .from('orders')
        .select('*')
        .eq('order.order.order_id', reference)
        .limit(1)
        .maybeSingle();
    ...
    if (alreadyProcessed) {
      console.log('Order already marked processed; skipping credit update. order:', order.order.order_id ?? order.order.order.order_id);
    ...
    const { error: updErr } = await supabaseAdmin
      .from('orders')
      .update(updates)
      .eq('order.order.order_id', order.order.order.order_id);
    ...
    if (!order.client_id) {
      console.warn('Order has no client_id; cannot credit customer. Order id:', order.order.order_id);
      res.status(200).json({ ok: true, note: 'no-client-id' });
      return;
    }

    if (!creditsToAdd) {
      console.warn('No credits amount found on order or metadata; skipping credit update. order:', order.order.order_id);
    ...
    if (creditSuccess) {
      ...
      console.warn('Order processed but credits not applied. Order id:', order.order.order_id);
